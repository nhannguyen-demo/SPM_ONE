      ...
      for (const eq of plant.equipment) {
        ...
        for (const tab of eq.tabs ?? []) {
          results.push({
            id: `dash-${eq.id}-${tab}`,
            label: tab,
            sublabel: `${eq.name} · ${site.name} › ${plant.name}`,
            kind: "dashboard",
            siteId: site.id,
            plantId: plant.id,
            equipmentId: eq.id,
...
const AI_NOTICES = [
  {
    ...
    title: "Coke Drum thermal cycling variance",
    body: "Thermal cycling variance is trending 12% above the 3-month rolling average. Review the Fatigue dashboard for detailed cycle count data.",
    location: "Coke Drum › Fatigue",
  },
  {
    severity: "warning" as const,
    title: "Pump ND.1X02 ΔP anomaly",
    body: "Differential pressure readings have been intermittently elevated over the past 72 hours. Inspection of the performance trend is recommended.",
    location: "Pump ND.1X02 › Pump Performance",
  },
  {
    severity: "info" as const,
    title: "HCU Maintenance window approaching",
    body: "Scheduled maintenance for the HCU is due in approximately 8 days based on last service records in the Maintenance dashboard.",
    location: "HCU › Maintenance",
  },
]
...
const AI_ACTIONS = [
  {
    ...
    action: "Check Coke Drum Fatigue dashboard",
    reason: "Thermal cycling variance trending above threshold.",
    dashboardKind: "Coke Drum — Fatigue",
  },
  {
    icon: <Activity className="w-4 h-4 text-rose-500" />,
    action: "Review Pump ND.1X02 performance trends",
    reason: "Elevated ΔP detected in recent sensor data.",
    dashboardKind: "Pump ND.1X02 — Pump Performance",
  },
  {
    icon: <Activity className="w-4 h-4 text-blue-500" />,
    action: "Inspect HCU Maintenance schedule",
    reason: "Service window approaching based on last recorded date.",
    dashboardKind: "HCU — Maintenance",
  },
]
...
/* ═══════════════════════════════════════════════════════════════════════════
   SHARED — Dashboard Card Row (used by Recent + Favourite)
   ═══════════════════════════════════════════════════════════════════════════ */
...
function RecentDashboardsModule() {
  ...
  const handleCardClick = (card: (typeof dashboardCards)[0]) => {
    ...
    const site = sites.find((s) =>
      s.plants.some((p) => p.equipment.some((e) => e.id === card.equipId))
    )
...
function FavouriteDashboardsModule() {
  ...
  const LIMIT = 6
  const visibleIds = showAll ? favouriteDashboardIds : favouriteDashboardIds.slice(0, LIMIT)
  const hasMore = favouriteDashboardIds.length > LIMIT
  ...
  return (
    <div>
      <DashboardCardRow
        cardIds={visibleIds}
        emptyMessage="No favourited dashboards yet — click the bookmark icon on any equipment dashboard to add it here."
        onCardClick={handleCardClick}
      />
      {hasMore && (
        <button
        ...
        >
          {showAll ? (
            ...
            <><ChevronDown className="w-4 h-4" /> Show {favouriteDashboardIds.length - LIMIT} more</>
          )}
        </button>
      )}
    </div>
...
function DocumentsModule() {
  ...
  const filtered = useMemo(() => {
    return userDocuments.filter((doc) => {
      ...
      if (assetFilter !== "All") {
        if (assetFilter.startsWith("site-")) {
          const siteId = assetFilter.slice(5)
          if (doc.siteId !== siteId) return false
        } else if (assetFilter.startsWith("plant-")) {
          const plantId = assetFilter.slice(6)
          if (doc.plantId !== plantId) return false
        } else if (assetFilter.startsWith("equip-")) {
          const equipId = assetFilter.slice(6)
          if (doc.equipmentId !== equipId) return false
        }
      }
