...
- ✅ Home view with modules: Global Search, AI Summary, Recents, Favourites, Change Log, Documents preview.
...
- ✅ Centralized What-If Tool v2 (scenario list, configure/run/history/results/compare, animated run steps).
...
- Home recents/favourites are in-memory state (reset on hard refresh by design today).
...
- Name clashes and nomenclature drift (`whatif` vs `WhatIf` vs `WIS`; rail module key/label mismatches).
...
3) Maintainability
- Very large component files (`equipment-dashboard.tsx`, `whatif-tool-view.tsx`, `home-view.tsx`, `sidebar.tsx`) need decomposition.
- Duplicate implementations:
  - Documents UI logic in both Home module and Documents Tool.
  - Equipment tab-strip rendering duplicated in normal/expanded dashboard modes.
  - Repeated card navigation handlers in Home modules.
  - Duplicate hook files (`hooks/*` vs `components/ui/*` equivalents).
