...
- ✅ Centralized What-If Scenario Tool v2 (configure/run/history/results/compare, animated run steps). Scenario selection uses an **inline dropdown** at the top of the page (same pattern as Data & Jobs equipment filter) — the legacy left-side panel is removed. Scenario **Overview** shows a short `description` under **About this Scenario** only (no subtitle under the scenario title; no long-form upload/outputs copy block).
